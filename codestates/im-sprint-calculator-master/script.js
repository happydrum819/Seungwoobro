...
buttons.addEventListener('click', function (event) {
  ...
  if (target.matches('button')) {
    if (action === 'number') {
      // previous key를 사용할 수 있는 것
      if(display.textContent === '0' || previousKey === operatorForAdvanced){
    ...
    if (action === 'decimal') {
      ...
      if(previousKey === operatorForAdvanced && !firstNum){
    ...
    if (action === 'calculate') {
      let result = calculate(previousNum, operatorForAdvanced, firstNum)
